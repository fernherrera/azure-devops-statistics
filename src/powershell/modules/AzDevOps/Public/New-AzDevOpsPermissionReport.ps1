function New-AzDevOpsPermissionReport
{
    <#
    .SYNOPSIS
    ...
    New-AzDevOpsPermissionReport -Session 'mySession' -InputObject @{
            descriptors = @()
            reportName = 'myFirstPermissionReport'
            resources = @(
                @{
    ...
    {
        $body = $InputObject
        $apiEndpoint = Get-AzDevOpsApiEndpoint -ApiType 'permissions'
        $setAzDevOpsUriSplat = @{
