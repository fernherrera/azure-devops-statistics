function Get-AzDevOpsGroupMembershipList
{
    <#
    .SYNOPSIS
    ...
    Use Get-AzDevOpsGroup with the descriptor to identify the group name. 
    ...
    $group = Get-AzDevOpsGroupList -Session $session | Select-Object -First 1
    $memberList = Get-AzDevOpsGroupMembershipList -Session $session -SubjectDescriptor $group.descriptor
    Get-AzDevOpsGroup -Session $session -GroupDescriptor $memberList.memberDescriptor
    ...
    process
    {
        $apiEndpoint = (Get-AzDevOpsApiEndpoint -ApiType 'graph-memberships') -f $SubjectDescriptor
